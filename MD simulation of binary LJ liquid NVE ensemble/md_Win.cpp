	...
	for(unsigned int i=0; i<np; i++) {
		...
		for(unsigned int j=0; j<np; j++) {
			...
			if (r < rcut) {
				...
				if(type==2) {             // i=1.0, j=1.0
					epsilonij = epsilon.x;
					sigmaij = sigma.x;
				}
				else if(type==3) {       // i=1.0, j=2.0; or i=2.0, j=1.0
					epsilonij = epsilon.y;
					sigmaij = sigma.y;
				}
				else if(type==4) {       // i=2.0, j=2.0
					epsilonij = epsilon.z;
					sigmaij = sigma.z;
				}
...
int main(int argc, char **argv) {
	...
	printf("Starting NVE simulation with %d atoms for %d steps.\n", np, nsteps);
